...

{
		...
		mysqli_query($link,"UPDATE auditoriamedica SET
		...
		$mensaje = 	"Auditoria actualizada";
...

			{
				...
				mysqli_query($link,"INSERT INTO auditoriamedica_adjuntos(IDAuditoriamedica, AudUsrAlta, AudFecAlta)
...

				{
					...
					$sql="UPDATE auditoriamedica_adjuntos set Archivo='$archivo' WHERE IDAdjunto=$id";
...

{
	...
	mysqli_query($link,"DELETE FROM auditoriamedica_adjuntos WHERE IDAdjunto=". $_POST["id_adjunto"]);
...

{
	...
	$sql = mysqli_query($link,"SELECT IDAuditoria, Observaciones, IDEstadoAuditoria, FechaEnvio, ACC_Nombre, ACC_Telefono, instituciones.Nombre as NombreInst FROM auditoriamedica, denuncias, instituciones WHERE IDEstadoAuditoria='1' AND  auditoriamedica.IDDenuncia=denuncias.ID AND instituciones.ID=denuncias.DE_IDInstitucion AND auditoriamedica.IDDenuncia=$id");
	  ...
	  <table width="95%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td height="26" colspan="4" align="left" class="tahoma11"><p>Accidentado: <?php echo $accidentado ?></p>
			  ...
			  <p>Fecha Solicitud Auditoría: <?php echo $fechaenvio?></p>
            </td>
        </tr>
        <tr>
        ...
        <tr>
          <td colspan="3" align="left" class="tahoma11">Estado de la Auditor&iacute;a:<br>
            <select name="cmbEstado" id="cmbEstado" class="tahoma12" style="width:120px ">
                <option value="1" <?php if($estado=="1") echo "selected"?> selected="selected">En Proceso</option>
        ...
        <?php  // listo los archivos asociados
				...
				$sql = mysqli_query($link,"SELECT * FROM auditoriamedica_adjuntos WHERE IDAuditoriamedica=$id_auditoria");
